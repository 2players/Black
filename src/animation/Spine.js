...
class Spine extends DisplayObject {
  ...
  constructor(name, texturesPath = '') {
    ...
    for (let skinName in json.skins) {
      ...
      for (let slotName in skin) {
        ...
        for (let entryName in slot) {
          ...
          if (attachment.type === 'point') continue
          ...
          if (attachment.name) textureName = attachment.name

          if (regions[textureName]) continue

          regions[textureName] = spineAtlas.addTexture(
            textureName,
            AssetManager.default.getTexture(texturesPath + textureName)
          )
        }
      }
    }
    ...
    this.mState.addListener({
      complete: x => this.post('animationComplete', x.animation.name),
    })
  }
  ...
  setTransition(from, to, loop, dur = 0, viseversaDur = 0) {
    let h = t => {
      if (t.animation.name === from) this.play(to, loop)
    }

    this.mState.addListener({ complete: h })

    if (dur > 0) this.mStateData.setMix(from, to, dur)
    if (viseversaDur > 0) this.mStateData.setMix(to, from, viseversaDur)
  }
      ...
      if (attachment instanceof spine.RegionAttachment) {
        ...
        if (region) {
          if (
            ...
            slot.currentSpriteName !== region.name
          ) {
        ...
        wrapper.alpha =
          this.mSkeleton.color.a * slot.color.a * attachment.color.a
